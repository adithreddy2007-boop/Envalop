...
import { saveCard, loadCard, generateUniqueCode } from './storage.js';
...
async function copyShareLink(btnEl) {
  ...
  try {
    ...
    // Fallback: a temporary off-screen input, since there's no visible link field to select from.
    try {
  ...
  if (btnEl) {
    ...
    btnEl.textContent = ok ? 'Link copied!' : 'Copy failed';
...
function onAppClick(e) {
  ...
  switch (action) {
    ...
    case 'copyLink': copyShareLink(t); break;
